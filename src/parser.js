...
export function parseSheet(sheet, sheetName) {
    ...
    const columnValues = rawHeaders.map((_, colIndex) =>
    ...
    const filteredHeaders = normalizedHeaders.filter((_, index) =>
        !emptyColumnIndices.has(index) && normalizedHeaders[index] !== ''
    );
