...
function mergeMetricKeys(detectedKeys, overrideKeys, scenarioData) {
    ...
    for (const key of overrideKeys) {
        const existsInData = scenarioData.some(day =>
            Object.prototype.hasOwnProperty.call(day.metrics, key)
        );
        if (existsInData) {
            merged.add(key);
        }
...
export function extractQueueTimeSeries(scenarioData, scenarioName) {
    ...
    const queueMetrics = Object.keys(firstDay.metrics).filter(key =>
        key.endsWith('_level')
    );
...
export function extractProcessTimeSeries(scenarioData, scenarioName) {
    ...
    const firstDay = scenarioData[0];
    const processMetrics = Object.keys(firstDay.metrics).filter(key =>
    ...
    for (const metricKey of mergedProcessMetrics) {
        ...
        const utilizationKey = Object.keys(firstDay.metrics).find(key =>
            key.startsWith(baseProcessName) && (key.includes('workload') || key.includes('utilization') || key.includes('%'))
        );
...
export function buildTabPayload(scenarioName, scenarioData, bottleneckInfo, queueMetrics, processMetrics) {
    ...
    for (const process of processMetrics) {
        if (process.utilizationRate !== null) {
            ...
            const utilizationKey = Object.keys(firstDay.metrics).find(key =>
                key.startsWith(baseMetricKey) && (key.includes('workload') || key.includes('utilization') || key.includes('%'))
            );
