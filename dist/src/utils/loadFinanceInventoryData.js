...
export async function loadFinanceInventoryData() {
    try {
        ...
        const totalDays = meta.sheets?.financial?.days ||
        ...
        for (let day = 0; day < totalDays; day++) {
            ...
            financePromises.push(fetch(`/output/financial/day_${dayStr}.json`, {
                cache: "no-store",
                headers: { Accept: "application/json" },
            }).then((res) => (res.ok ? res.json() : null)));
            inventoryPromises.push(fetch(`/output/inventory/day_${dayStr}.json`, {
                cache: "no-store",
                headers: { Accept: "application/json" },
            }).then((res) => (res.ok ? res.json() : null)));
        }
